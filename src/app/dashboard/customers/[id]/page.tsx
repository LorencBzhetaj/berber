...
import { getCustomerDetail, getSettings } from "@/server/data";
...
export default async function CustomerDetailPage({
  ...
  const [customer, settings] = await Promise.all([getCustomerDetail(id), getSettings()]);
      ...
      <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <MiniStat label="Total spent" value={formatMoney(s.totalSpent, cur)} />
            <MiniStat label="Last visit" value={s.lastVisit ? formatDate(s.lastVisit) : "—"} />
            <MiniStat label="Favourite barber" value={s.favoriteBarber ?? "—"} icon={User} />
            <MiniStat label="Favourite service" value={s.favoriteService ?? "—"} icon={Scissors} />
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {customer.appointments.map((a) => (
                  <TableRow key={a.id}>
                    <TableCell>
                      <div className="font-medium">{formatDate(a.startDateTime)}</div>
                      <div className="text-xs text-muted-foreground">{formatTime(a.startDateTime)}</div>
                    </TableCell>
                    <TableCell>{a.service.name}</TableCell>
                    <TableCell className="text-muted-foreground">{a.barber.displayName}</TableCell>
                    <TableCell className="font-medium">{formatMoney(a.price, cur)}</TableCell>
                    <TableCell>
